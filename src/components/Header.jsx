...
const Header = ({ title, displayModalHamburger, displayModalSearch }) => {
  return (
    <header className='flex justify-between items-start mt-3'>
      <button className='w-10 h-10 sm:hidden' onClick={displayModalHamburger}>
        <RxHamburgerMenu className='size-8' />
      </button>
      <h1 className='text-4xl text-textOnLight h-10 font-semibold'>{title}</h1>
